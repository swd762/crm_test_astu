<template>
    <div>
        <h1 class="mb-8 font-bold text-3xl">Счета</h1>
        ...
        <div class="bg-white rounded-md shadow overflow-x-auto">
            <table class="w-full whitespace-nowrap">
                ...
                <tr v-for="bill in bills.data" :key="bill.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center focus:text-indigo-500" :href="route('bills.edit', bill.id)">
                            {{ bill.bill_number }}
<!--                            <icon v-if="bill.deleted_at" name="trash" class="flex-shrink-0 w-3 h-3 fill-gray-400 ml-2" />-->
                        </inertia-link>
                    </td>
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center" :href="route('bills.edit', bill.id)" tabindex="-1">
                            {{ bill.amount }}
                        </inertia-link>
                    </td>
                    <td class="border-t">
    ...
    watch: {
        form: {
            ...
            handler: throttle(function() {
                this.$inertia.get(this.route('bills'), pickBy(this.form), { preserveState: true })
            }, 150),
        },
    },
    methods: {
